...
export const sendBirthdayEmail = async (email, name) => {
  ...
  const formattedExpDate = expDate.toLocaleDateString('fr-CA', { day: 'numeric', month: 'long', year: 'numeric' });
  ...
  const payload = {
    ...
    htmlContent: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px; text-align: center;">
        ...
        <p style="font-size: 1.1em; color: #5A4A42;">
          Pour célébrer, nous voulons vous offrir un <strong>breuvage 16 oz</strong> de votre choix ! 🍹☕
        </p>
        ...
        <div style="background-color: #FCE4EC; padding: 15px; border-radius: 8px; margin: 30px 0; border: 2px dashed #E91E63;">
          ...
          <p style="margin-bottom: 0; font-size: 1.2em; font-weight: bold; color: #2C1810;">1 breuvage 16 oz au choix !</p>
          <p style="margin-top: 8px; margin-bottom: 0; font-size: 0.85em; color: #C2185B;">Les formats plus grands, modifications et extras sont offerts avec supplément. Une seule utilisation. Non monnayable et non combinable avec une autre promotion.</p>
        </div>
        
        <p style="color: #666;">
          Votre cadeau est valide pendant 7 jours, jusqu'au <strong>${formattedExpDate}</strong>.<br>
          Venez nous voir en boutique et mentionnez simplement que c'est votre anniversaire. Nous serons heureux de préparer votre breuvage favori.
        </p>
        
        <div style="background-color: #880E4F; padding: 15px; border-radius: 8px; margin: 20px 0; text-align: left; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
          <p style="margin: 0; color: #FFFFFF; font-size: 1.05em;">
            <strong>👯‍♀️ Pssst... ne venez pas seul(e) !</strong><br>
            Amenez vos proches pour célébrer avec vous et ils obtiendront <strong>10 % de rabais</strong> sur leur commande !
          </p>
        </div>
        
        <div style="text-align: center; margin: 30px 0;">
          <a href="https://cafenamasthesherbrooke.ca" style="background-color: #E91E63; color: white; padding: 14px 28px; text-decoration: none; border-radius: 30px; font-weight: bold; font-size: 1.1em; box-shadow: 0 4px 10px rgba(233, 30, 99, 0.3);">Choisir mon breuvage 🎁</a>
        </div>
