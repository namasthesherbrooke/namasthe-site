  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (data.success && data.menu) {
        // --- INJECTION DU MENU D'AUTOMNE COMME RECETTES (MODIFIERS) ---
        const fallMegaTheList = {
          id: 'ml_fall_mega',
          name: '🍂 Spécialités d\'Automne',
          min: 0, max: 1,
          modifiers: [
            { id: 'f13', name: 'Dragon d\'Automne (Lotus Gold, fruit du dragon, orange, grenade)', price: 0 },
            { id: 'f14', name: 'Moka blanc épicé (Ananas, latté citrouille, colada)', price: 0 },
            { id: 'f15', name: 'Dirty Red Chai (Grenade, cerise, pêche, base chai)', price: 0 },
            { id: 'f16', name: 'Croustade aux pommes (Lotus Gold, pomme, caramel, cannelle)', price: 0 }
          ]
        };
        ...
        const fallShakeList = {
          ...
          min: 0, max: 1,
          modifiers: [
            { id: 'f1', name: 'Cheesecake pomme épicée (Cheesecake, pomme épicée, cannelle)', price: 0 },
            { id: 'f2', name: 'Banana Bread Chai (Banane, chai, orange, mangue)', price: 0 },
            { id: 'f3', name: 'Cheesecake citrouille (Cheesecake, citrouille, épices)', price: 0 },
            { id: 'f4', name: 'Gâteau citrouille praliné (Gâteau citrouille, praliné, épices)', price: 0 }
          ]
        };
        ...
        const fallCafeList = {
          ...
          min: 0, max: 1,
          modifiers: [
            { id: 'f5', name: 'Chai Cheesecake pomme épicée (Chai, cheesecake, pomme épicée)', price: 0 },
            { id: 'f6', name: 'Pumpkin Cream Chai (Chai, citrouille, épices, crème)', price: 0 },
            { id: 'f7', name: 'Dirty Chai Latté érable (Chai, érable, espresso)', price: 1.50 },
            { id: 'f8', name: 'Chai Latté Cinnamon Roll (Chai, brioche cannelle, cassonade)', price: 0 },
            { id: 'f9', name: 'Café Latté Pumpkin Spice (Espresso, citrouille, épices)', price: 0 },
            { id: 'f10', name: 'Café Latté Tarte aux pommes caramel (Espresso, pomme épicée, caramel)', price: 0 },
            { id: 'f11', name: 'Café Latté pacane caramélisée (Espresso, pacane, caramel)', price: 0 },
            { id: 'f12', name: 'Café Moka blanc cannelle (Espresso, chocolat blanc, cannelle)', price: 0 }
          ]
        };

        // Ajouter ces listes aux modifierLists globaux
        data.menu.modifierLists.push(fallMegaTheList, fallShakeList, fallCafeList);

        // Assigner ces listes aux produits de base correspondants
        data.menu.items.forEach(item => {
          const name = item.name.toLowerCase();
          if (!item.modifier_lists) item.modifier_lists = [];
          
          if (name.includes('méga thé') || name.includes('mega the') || name.includes('lotus')) {
            if (!item.modifier_lists.includes('ml_fall_mega')) item.modifier_lists.unshift('ml_fall_mega');
          } else if (name.includes('shake')) {
            if (!item.modifier_lists.includes('ml_fall_shake')) item.modifier_lists.unshift('ml_fall_shake');
          } else if (name.includes('café') || name.includes('latté') || name.includes('chai') || name.includes('latte')) {
            if (!item.modifier_lists.includes('ml_fall_cafe')) item.modifier_lists.unshift('ml_fall_cafe');
          }
        });
        // ------------------------------------
